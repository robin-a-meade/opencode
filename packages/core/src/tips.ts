export type TipPart = { text: string; highlight: boolean }
...
export function parse(tip: string): TipPart[] {
  const parts: TipPart[] = []
...
export const NO_MODELS_TIP = "Run {highlight}/connect{/highlight} to add an AI provider and start coding"
export const NO_MODELS_PARTS = parse(NO_MODELS_TIP)
...
const BUILTIN_THEME_COUNT = 34

export const INPUT_UNDO_TIP: Tip = (shortcuts) => press(shortcuts.inputUndo(), "to undo changes in your prompt")
export const TERMINAL_SUSPEND_TIP: Tip = (shortcuts) =>
...
export const TIPS: Tip[] = [
  ...
  (shortcuts) => `Use ${commandText("/themes", shortcuts.themeList())} to switch between ${BUILTIN_THEME_COUNT} built-in themes`,
